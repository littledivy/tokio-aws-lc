...
use aws_lc_sys::{
    BIO_clear_retry_flags, BIO_new, BIO_set_retry_read, BIO_set_retry_write, BIO,
...
impl BioMethod {
    fn new<S: Read + Write>() -> Result<BioMethod, ()> {
...
pub fn new<S: Read + Write>(stream: S) -> Result<(*mut BIO, BioMethod), ()> {
...
impl BIO_METHOD {
    fn new<S: Read + Write>() -> Result<BIO_METHOD, ()> {
        use ffi::{
        ...
        unsafe {
            ...
            BIO_meth_set_write(method.0, Some(bwrite::<S>));
            BIO_meth_set_read(method.0, Some(bread::<S>));
            BIO_meth_set_puts(method.0, Some(bputs::<S>));
            BIO_meth_set_ctrl(method.0, Some(ctrl::<S>));
            BIO_meth_set_create(method.0, Some(create));
            BIO_meth_set_destroy(method.0, Some(destroy::<S>));
            Ok(method)
        }
    }
